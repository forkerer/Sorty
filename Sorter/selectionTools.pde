...
int calcImageX(int x) {
  return (int)((x - (width-newWidth) / 2) * ((float)Scene.image.width / newWidth));
}

int calcScreenX(int x) {
  return (int)((x * (newWidth / (float)Scene.image.width)) + ((width-newWidth) / 2));
}

int calcImageY(int y) {
  return (int)((y - (height-newHeight) / 2) * ((float)Scene.image.height / newHeight));
}

int calcScreenY(int y) {
  return (int)((y * (newHeight / (float)Scene.image.height)) + ((height-newHeight) / 2));
}